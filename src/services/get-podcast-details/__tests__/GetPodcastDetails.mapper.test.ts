...
describe("Services - GetPodcastDetails - GetPodcastDetails mapper", () => {
  it("should map the api response to a podcast", async () => {
    const apiResponse = getPodcastDetailMock;
    const podcast: PodcastDetails = await mapApiResponseToPodcast(JSON.parse(apiResponse.contents));
      ...
      episodes: [
        {
          ...
          releaseDate: new Date("2024-09-11T07:00:00Z").toISOString(),
